...
class BiconomyExchange(ExchangePyBase):
    ...
    async def get_last_traded_prices(self, trading_pairs: List[str]) -> Dict[str, float]:
        prices = await self.get_all_pairs_prices()
        result: Dict[str, float] = {}
        symbol_map = {await self.exchange_symbol_associated_to_pair(tp): tp for tp in trading_pairs}
        for ticker in prices:
            symbol = ticker.get("symbol")
            trading_pair = symbol_map.get(symbol)
            if trading_pair:
                result[trading_pair] = float(ticker.get("last", 0))
        return result
